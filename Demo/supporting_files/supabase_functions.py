...
def fetch_data_from_supabase_specific_1(table_name, columns=None, filters=None, order_by=None, limit=None, offset=0):
    """
    Fetch data from Supabase table with optional filters, sorting, limit, and offset.
    """
    ...
    # Apply limit + offset using .range()
    ...
    if response.data is not None:
        ...
        raise Exception("Error fetching data: " + str(response.error_message))

import pandas as pd
import uuid

def sync_customer_tracking_incremental(limit=500, offset=0):
    """
    Incrementally sync Customer_Tracking from Tracking_Visitors in batches.
    - Fetches only `limit` rows starting from `offset`.
    - Uses last updated timestamp to fetch new visitors.
    - Merges with existing Customer_Tracking.
    """
    ...
    # 2️⃣ Fetch batch of Tracking_Visitors using offset + limit
    filters = {"Visited_at": ("gt", last_updated.isoformat())} if last_updated else None

    df = fetch_data_from_supabase_specific_1(
        "Tracking_Visitors",
        columns=[
            "Customer_ID", "Customer_Email", "Customer_Mobile", "Customer_Name",
            "Visitor_ID", "Session_ID", "Event_Type", "UTM_Campaign",
    ...
    if df.empty:
        print("✅ No new tracking data to sync (or done syncing).")
        return {"status": "done", "rows_fetched": 0, "next_offset": offset}
    ...
    # 8️⃣ Generate distinct_ids for new customers
    if "distinct_id" not in agg_df.columns:
        agg_df["distinct_id"] = None
    agg_df["distinct_id"] = agg_df["distinct_id"].fillna([str(uuid.uuid4()) for _ in range(len(agg_df))])
    ...
    print(f"✅ Synced {len(agg_df)} customers (offset={offset}, limit={limit}).")
    return {"status": "ok", "rows_fetched": len(df), "next_offset": offset + limit}
